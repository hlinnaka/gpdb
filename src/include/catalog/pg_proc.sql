...
CREATE FUNCTION cdblegacyhash_uuid(uuid) RETURNS int4 LANGUAGE internal IMMUTABLE STRICT AS 'cdblegacyhash_uuid' WITH (OID=6169, DESCRIPTION="Legacy cdbhash function");
CREATE FUNCTION cdblegacyhash_complex(complex) RETURNS int4 LANGUAGE internal IMMUTABLE STRICT AS 'cdblegacyhash_complex' WITH (OID=6170, DESCRIPTION="Legacy cdbhash function");
CREATE FUNCTION cdblegacyhash_anyrange(anyrange) RETURNS int4 LANGUAGE internal IMMUTABLE STRICT AS 'cdblegacyhash_anyrange' WITH (OID=6171, DESCRIPTION="Legacy cdbhash function");

CREATE FUNCTION hashtid(tid) RETURNS int4 LANGUAGE internal IMMUTABLE STRICT AS 'hashtid' WITH (OID=6172, DESCRIPTION="hash function for tid");

CREATE FUNCTION bithash(bit) RETURNS int4 LANGUAGE internal IMMUTABLE STRICT AS 'bithash' WITH (OID=6173, DESCRIPTION="hash function for bit");
CREATE FUNCTION bithash(varbit) RETURNS int4 LANGUAGE internal IMMUTABLE STRICT AS 'bithash' WITH (OID=6174, DESCRIPTION="hash function for bit varbit");


CREATE FUNCTION pg_get_table_distributedby(oid) RETURNS text LANGUAGE internal STABLE STRICT AS 'pg_get_table_distributedby' WITH (OID=6175, DESCRIPTION="deparse DISTRIBUTED BY clause for a given relation");
