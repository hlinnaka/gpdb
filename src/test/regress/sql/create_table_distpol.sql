...
select distkey, distclass from gp_distribution_policy where localoid='distpol_bar'::regclass;
...
select distkey, distclass from gp_distribution_policy where localoid='r3_1'::regclass;

create temp table r3_2 on commit delete rows as select 10 as a2, 20 as b2, 30 as c2, 40 as d2;
select distkey, distclass from gp_distribution_policy where localoid='r3_2'::regclass;

create temp table r3_3 on commit drop as select 10 as a3, 20 as b3, 30 as c3, 40 as d3;
select distkey, distclass from gp_distribution_policy where localoid='r3_3'::regclass;
...
select distkey, distclass from gp_distribution_policy where localoid = 'distpol_staff_member'::regclass;

CREATE TABLE distpol_student (
  gpa      float8
) INHERITS (distpol_person);
select distkey, distclass from gp_distribution_policy where localoid = 'distpol_student'::regclass;

CREATE TABLE distpol_stud_emp (
  percent  int4
) INHERITS (distpol_staff_member, distpol_student);
select distkey, distclass from gp_distribution_policy where localoid = 'distpol_stud_emp'::regclass;
...
select distkey, distclass from gp_distribution_policy where localoid = 'distpol_person'::regclass;

CREATE TABLE distpol_person_copy (LIKE distpol_person);
select distkey, distclass from gp_distribution_policy where localoid = 'distpol_person_copy'::regclass;
