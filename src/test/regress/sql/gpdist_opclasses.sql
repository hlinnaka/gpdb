...
-- In this scenario, the new datatype is not hashable.
-- The current behavior is that this is allowed, as long as the table
-- is completely empty. The table is turned into randomly distributed
-- in that case. If there is any data, then altering a column that's
-- part of the distribution key is not allowed.
CREATE TABLE distpol_typechange (p text) DISTRIBUTED BY (p);
ALTER TABLE distpol_typechange ALTER COLUMN p SET DATA TYPE point USING p::point;
