			  ...
			  Oid shelloid)
{
	...
	if (IsBinaryUpgrade && (relation_oid_hash != NULL) )
	{
		...
		Relation name_space_desc = heap_open(RelationGetNamespace(pg_type_desc));

		snprintf(searchName, sizeof(searchName), "%s.%s_type", RelationGetName(name_space_desc), typeName);
