 ...
 *    push_partition_selector_candidate_for_join() decides if it would be
 *    possible to put a partition selector to the inner side of the join,
 *    and if so, it records information about the join in a stack, in
 *    root->partition_selector_candidates. (In this phase, we don't
 ...
 *    a partitioned table on the outer side of the join,
 *    create_append_plan() calls make_partition_join_pruneinfos() to check
 *    if any of the partitions could be pruned based on any of the joins
 *    recorded in the root->partition_selector_candidates stack. If so, it
 *    builds the pruning steps into a PartitionPruneInfo struct, and adds
 *    it to the stack entry. At this point, we have decided that a Partition
 *    Selector will be used at the join.
 *
 * 3. After creating the outer side of the join, the create_*join_plan()
 *    calls pop_and_inject_partition_selectors(). If we decided to use add
 *    any Partition Selectors at this join (decided in the previous step),
 *    the Partition Selectors are added to the inner side of the join.
 *
 *
 ...
 * pruning doesn't affect cost estimates or the chosen plan shape. That's not great...
 *
...
push_partition_selector_candidate_for_join(PlannerInfo *root, JoinPath *join_path)
{
	 ...
	 * This is just a quick check to see if there is any hope.
	 *
	 * XXX: Could we check for overlap between inner and outer relids perhaps,
	 * to do a slightly more thorough check?
	 */
