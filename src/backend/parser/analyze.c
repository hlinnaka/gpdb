...
setQryDistributionPolicy(IntoClause *into, Query *qry)
{
	...
	else
	{
		...
		foreach(lc, dist->keyCols)
		{
			...
			// FIXME: Consider just calling GetIndexOpclass(). Currently, there are
			// a few datatypes that have a legacy hash opclass, but no normal,
			// default, hash opclass. GetIndexOpclass() throws an error in that case.
			// If we created default hash opclasses for those datatypes, then we
			// could just let GetIndexOpclass() handle it.
			// FIXME: the error messages in GetIndexOpclass talk about indexes.
			keytype = exprType((Node *) tle->expr);
			if (ielem->opclass)
				keyopclass = GetIndexOpClass(ielem->opclass, keytype, "hash", HASH_AM_OID);
			else
				keyopclass = cdb_default_distribution_opclass_for_type(keytype);
