...
apply_motion(PlannerInfo *root, Plan *plan, bool *needToAssignDirectDispatchContentIds)
{
	...
	*needToAssignDirectDispatchContentIds = false;
	...
	while (state.subplan_workingQueue)
	{
		...
		if (subplan->flow->locustype != CdbLocusType_Upper &&
...
apply_motion_mutator(Node *node, ApplyMotionState *context)
{
	...
	if (!is_plan_node(node))
	{
		...
		if (IsA(node, SubPlan))
		{
			...
			if (!spexpr->is_initplan && context->sliceDepth > sstate->sliceDepth)
			{
				...
				if (context->currentPlanFlow->locustype == CdbLocusType_Upper)
	...
	if (IsA(plan, Motion))
	{
		...
		/*
		 * If this is a Motion node in a correlated SubPlan, where we bring the result
		 * to the parent, the Flow will be marked with CdbLocusType_Upper. Because
		 * we didn't know whether the parent is distributed, replicated, or a single
		 * QD/QE process, when the subquery was planned, we fix that up here. Modify
		 * the Motion node so that it brings the result to the parent.
		 */
		if (plan->flow->locustype == CdbLocusType_Upper)
		{
