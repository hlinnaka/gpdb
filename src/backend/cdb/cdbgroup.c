		...
		MppGroupContext *ctx)
{
	...
	if (ctx->subplan == NULL)
	{
		...
		foreach(lc, current_pathkeys)
		{
			PathKey *pk = (PathKey *) lfirst(lc);

			if (!cdbpullup_findPathKeyExprInTargetList(pk, sub_tlist))
				break;

			current_pathkeys = lappend(current_pathkeys, path->pathkeys);
		}
...
Plan *add_motion_to_dqa_child(Plan *plan, PlannerInfo *root, bool *motion_added)
{
	...
	*motion_added = false;
	...
	List *pathkeys = make_pathkeys_for_sortclauses(root, root->parse->groupClause, plan->targetlist, false);
