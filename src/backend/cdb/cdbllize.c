...
cdbparallelize(PlannerInfo *root, Plan *plan, bool *needToAssignDirectDispatchContentIds)
{
	...
	/* this should only be called at the to -level, not on a subquery */
...
parallelize_subplans(Plan *plan, PlanProfile *context)
{
	...
	for (int plan_id = 1; plan_id <= list_length(root->glob->subplans); plan_id++)
	{
		...
		else if (!spinfo->is_initplan)
		{
			Assert(subplan_plan->flow && subplan_plan->flow->flotype != FLOW_UNDEFINED);
			if (subplan_plan->flow->flotype != FLOW_SINGLETON)
			{
				subplan_plan = focusPlan(subplan_plan, false);
				subplan_plan->flow->locustype = CdbLocusType_Upper;

				subplan_plan = (Plan *) make_material(subplan_plan);
			}
		}
...
focusPlan(Plan *plan, bool stable)
{
	...
	if (stable && plan->flow->flotype != FLOW_SINGLETON)
		elog(ERROR, "FIXME: sorted focusPlan not supported");
